...
function requireOrg(req, res) {
  if (req.user.type !== 'org_user') { res.status(403).json({ error: 'Forbidden' }); return false; }
  return true;
}
...
router.get('/', auth, (req, res) => {
  ...
  const codes = db.prepare(`
    SELECT c.id, c.name, c.type, c.behavior, c.title, c.description, c.notification_message,
           c.mailbox_label, c.contact_email, c.contact_phone, c.scan_token, c.created_at,
...
router.post('/', auth, async (req, res) => {
  if (!requireOrg(req, res)) return;
  const { name, type, behavior, title, description, notification_message, mailbox_label, contact_email, contact_phone } = req.body;
  ...
  let qrDataUrl = null;
  if (type === 'QR') qrDataUrl = await QRCode.toDataURL(scanUrl, { width: 300 });

  db.prepare(`INSERT INTO codes (id, org_id, name, type, behavior, title, description, notification_message, mailbox_label, contact_email, contact_phone, qr_data_url, scan_token)
              VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .run(id, req.user.orgId, name, type, codeBehavior,
      title || null, description || null, notification_message || null,
      mailbox_label || null, contact_email || null, contact_phone || null, qrDataUrl, scanToken);

  return res.status(201).json({ id, name, type, behavior: codeBehavior, title, description, notification_message, mailbox_label, contact_email, contact_phone, scan_token: scanToken, scan_url: scanUrl, qr_data_url: qrDataUrl });
});
...
router.patch('/:id', auth, (req, res) => {
  ...
  const allowed = ['title', 'description', 'notification_message', 'mailbox_label'];
  const updates = [];
  const params = [];
  ...
  db.prepare(`UPDATE codes SET ${updates.join(', ')} WHERE id = ?`).run(...params);
...
router.get('/:id/qr', auth, (req, res) => {
  ...
  const imgBuffer = Buffer.from(code.qr_data_url.replace(/^data:image\/png;base64,/, ''), 'base64');
...
router.get('/:id/members', auth, (req, res) => {
  ...
  const members = db.prepare(`
    SELECT tm.id, tm.status, tm.paused, tm.vacation_until, tm.invited_at, tm.responded_at, eu.email
    FROM token_memberships tm JOIN end_users eu ON eu.id = tm.end_user_id
    WHERE tm.token_id = ? ORDER BY tm.invited_at DESC
  `).all(code.id);
...
router.post('/:id/fields', auth, (req, res) => {
  ...
  const id = uuidv4();
  db.prepare('INSERT INTO token_input_fields (id, token_id, label, placeholder, field_type, options_json, required, sort_order) VALUES (?,?,?,?,?,?,?,?)')
...
router.get('/:id/messages', auth, (req, res) => {
  ...
  const code = db.prepare('SELECT id FROM codes WHERE id = ? AND org_id = ?').get(req.params.id, req.user.orgId);
  if (!code) return res.status(404).json({ error: 'Code not found' });
  return res.json(db.prepare('SELECT channel, message_template FROM token_channel_messages WHERE token_id = ?').all(code.id));
});

router.put('/:id/messages', auth, (req, res) => {
  if (!requireOrg(req, res)) return;
  const code = db.prepare('SELECT id FROM codes WHERE id = ? AND org_id = ?').get(req.params.id, req.user.orgId);
  if (!code) return res.status(404).json({ error: 'Code not found' });

  const { channel, message_template } = req.body;
  ...
  db.prepare(`
    INSERT INTO token_channel_messages (id, token_id, channel, message_template)
    VALUES (?, ?, ?, ?)
    ON CONFLICT(token_id, channel) DO UPDATE SET message_template = excluded.message_template
  `).run(uuidv4(), code.id, channel, message_template);
  return res.json({ success: true });
});

router.delete('/:id/messages/:channel', auth, (req, res) => {
  if (!requireOrg(req, res)) return;
  const code = db.prepare('SELECT id FROM codes WHERE id = ? AND org_id = ?').get(req.params.id, req.user.orgId);
  if (!code) return res.status(404).json({ error: 'Code not found' });
  db.prepare('DELETE FROM token_channel_messages WHERE token_id = ? AND channel = ?').run(code.id, req.params.channel);
  return res.json({ success: true });
});
